...
| MP3 / M4A / AAC / FLAC / WAV / WMA / OGG / OPUS | MP3 / M4A / WAV / FLAC / WMA / OPUS | Stable | FFmpeg compatible | Common audio conversion path. MP3 uses `libmp3lame`; M4A uses AAC; WAV uses PCM; FLAC uses FLAC; WMA uses WMA v2 in ASF/WMA; OPUS uses `libopus` in Ogg. Start/end-second trimming can limit the exported range. MP3 offers CBR bitrate choices or VBR quality presets V0/V2/V4/V6; M4A/WMA/OPUS use selected bitrate when supported. Sample-rate, channel, reverse, fade, volume/mute, echo, and audio noise-reduction controls are applied when selected. OPUS natively operates at 48 kHz (RFC 6716 / RFC 7845) and supports {48k, 24k, 16k, 12k, 8k}; non-Opus rates like 44.1 kHz are automatically resampled to 48 kHz. WAV/FLAC ignore bitrate. |
...
- PNG, WEBP, GIF, ICO, and video cleanup are not connected in this
  milestone. HEIC/HEIF images can be inspected for EXIF/GPS metadata through ExifInterface, but in-place cleanup is not supported. Unsupported images may still show basic format/size/dimensions.
